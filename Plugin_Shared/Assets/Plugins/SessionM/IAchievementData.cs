...
public interface IAchievementData
{
	...
	int GetTimesEarned();
	/*!
	 * The current amount of this achievement the user has earned, but not yet claimed. Returns -1 if achievement is not accessed from UserData.GetAchievementsList().
	 */
	int GetUnclaimedCount();
	/*!
	 * The number of actions required to earn a new achievement. Returns -1 if achievement can't be earned in the current session.
	 */
	int GetDistance();
}
